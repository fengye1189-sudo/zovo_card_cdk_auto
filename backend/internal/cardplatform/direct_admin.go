...
func PublicDirectOrder(v map[string]any) map[string]any {
	out := publicScalars(v, "id client_request_id product plan status stage account_email email currency quoted_amount_minor final_amount_minor service_fee_minor service_fee_status funding_state funding_hold_status funding_card_amount_minor payment_status payment_fact renewal_status renewal_message will_renew subscription_will_renew total_cost_usd_minor actual_cost_usd_minor subscription_cost_usd_minor recharge_fee_usd_minor open_fee_allocated_usd_minor card_open_fee_allocated_usd_minor cost_complete cost_source message error_code created_at updated_at completed_at card_id")
	if raw, ok := v["cost_breakdown"].(map[string]any); ok {
		safe := publicScalars(raw, "total_usd_minor subscription_usd_minor service_fee_usd_minor recharge_fee_usd_minor open_fee_allocated_usd_minor complete source")
		if len(safe) > 0 {
			out["cost_breakdown"] = safe
		}
	}
	for _, k := range []string{"card_last_four", "card_number"} {
		if s, ok := v[k].(string); ok && len(s) >= 4 {
			tail := s[len(s)-4:]
			if _, e := strconv.Atoi(tail); e == nil {
				out["card_last_four"] = tail
				break
			}
		}
	}
...
func (c *Client) DirectOrderList(ctx context.Context, page int) (map[string]any, error) {
	...
	if json.Unmarshal(raw, &v) != nil || v.List == nil || v.Total == nil || *v.Total < 0 || len(v.List) > 20 {
		return nil, fmt.Errorf("invalid order list")
	}
	list := []map[string]any{}
	for _, o := range v.List {
		list = append(list, PublicDirectOrder(o))
	}
...
func (c *Client) DirectBoundOrderDetail(ctx context.Context, id int64, request string) (map[string]any, error) {
	if request == "" {
		return nil, fmt.Errorf("missing merchant request")
	}
	return c.directOrderDetail(ctx, id, request)
}
func (c *Client) directOrderDetail(ctx context.Context, id int64, request string) (map[string]any, error) {
	if id <= 0 {
		return nil, fmt.Errorf("invalid order id")
	}
	raw, e := c.DirectOrderStatus(ctx, id)
	if e != nil {
		return nil, e
	}
	var v struct {
	...
	if json.Unmarshal(raw, &v) != nil || v.Order["id"] != float64(id) || (request != "" && v.Order["client_request_id"] != request) {
	...
	for i, event := range v.Events {
		...
		events = append(events, publicScalars(event, "id created_at occurred_at category step public_code public_message status to_status payment_fact"))
	}
...
func (c *Client) DirectAdminAction(ctx context.Context, id int64, action string) (map[string]any, error) {
	if id <= 0 || (action != "cancel" && action != "cancel-renewal") {
		return nil, fmt.Errorf("invalid action")
	}
	raw, e := c.doOpenAPI(ctx, http.MethodPost, "/gpt-direct/orders/"+strconv.FormatInt(id, 10)+"/"+action, map[string]any{}, "")
	if e != nil {
		return nil, e
	}
	var v map[string]any
	if json.Unmarshal(raw, &v) != nil {
		return nil, fmt.Errorf("invalid action result")
	}
