...
func directReadError(c *gin.Context,e error) {
 ...
 if errors.As(e,&upstream) {switch upstream.HTTPStatus {
 case 401,403:localError(c,502,"Zovo 拒绝访问，请检查接入密钥、权限或服务器白名单");return
 case 404:localError(c,404,"找不到本 API 账号可访问的订单");return
 case 429:localError(c,429,"上游请求频繁，请稍后刷新");return
 }}
...
func AdminDirectDetail(c *gin.Context) {
 c.Header("Cache-Control","no-store");id,ok:=directID(c);if !ok{return}
 data,e:=cardplatform.NewFromSettings().DirectOrderDetail(c.Request.Context(),id);if e!=nil{directReadError(c,e);return};c.JSON(200,data)
}
func directActionAllowed(order map[string]any,action string) bool {
 status,_:=order["status"].(string)
 if action=="cancel" {return status=="queued" || status=="awaiting_card" || status=="funding_pending"}
...
func AdminDirectAction(c *gin.Context) {
 c.Header("Cache-Control","no-store");id,ok:=directID(c);if !ok{return}
 action:=c.Param("action");if action!="cancel" && action!="cancel-renewal" {localError(c,404,"操作不存在");return}
 var req struct {Confirmed bool `json:"confirmed"`; ExpectedStatus string `json:"expected_status"`}
 if !localBody(c,&req){return};if !req.Confirmed || req.ExpectedStatus==""{localError(c,400,"请先查看订单并确认操作");return}
 cli:=cardplatform.NewFromSettings();data,e:=cli.DirectOrderDetail(c.Request.Context(),id);if e!=nil{directReadError(c,e);return}
 order:=data["order"].(map[string]any)
 if order["status"]!=req.ExpectedStatus || !directActionAllowed(order,action) {localError(c,409,"当前订单状态不允许此操作，请刷新详情。已取消续费的订单无需重复取消。");return}
 cfg:=cardplatform.LoadConfig();key:=localHash(cfg.SiteBase+"|"+cfg.APIKey+"|"+strconv.FormatInt(id,10)+"|"+action)
 now:=time.Now().Unix()
 ...
 if e!=nil {localError(c,503,"操作记录暂不可用，未提交取消请求");return}
 n,_:=result.RowsAffected();if n!=1 {localError(c,409,"该操作正在处理、结果待核对或距离上次操作不足两分钟。请先刷新；结果不明时在 Zovo 核对，不要连续重试。");return}
 ...
 state:="done";if e!=nil{state="unknown"}
 _,saveErr:=db.DB.Exec("UPDATE direct_admin_actions SET state=? WHERE action_key=?",state,key)
 if e!=nil || saveErr!=nil {localError(c,502,"取消请求结果尚未确认，请刷新订单或到 Zovo 核对。本站不会自动重试，也不会重新付款。");return}
